...
from app.schemas.google_solar_building_insights_response_schema import GoogleSolarPanelConfig
...
def calculate_solar_potential(address: str, max_solar_panels_percentage: float) -> SolarPotentialResponseSchema:
...
def enrich_solar_panel_configs(solar_panel_configs):
    for solar_panel_config in solar_panel_configs:
        solar_panel_config.yearly_energy_ac_kwh = solar_panel_config.yearly_energy_dc_kwh * DC_TO_AC_RATE
        solar_panel_config.yearly_energy_ac_euro = solar_panel_config.yearly_energy_ac_kwh * AVG_COST_PER_KWH


def find_solar_panel_config_closest_to_max_panels(solar_panel_configs: [GoogleSolarPanelConfig],
                                                  max_panels_count: int,
                                                  max_solar_panels_percentage: float) -> GoogleSolarPanelConfig:
    target_panels_count = math.ceil(max_panels_count * (max_solar_panels_percentage / 100))
    return next(
        filter(lambda solar_panel_config: solar_panel_config.panels_count == target_panels_count, solar_panel_configs)
    )
